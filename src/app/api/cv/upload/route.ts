...
export async function POST(request: NextRequest) {
  try {
    ...
    const uniqueFileName = `cv/${session.user.id}/${templateName}/${timestamp}-${randomString}-${fileName}`;

    console.log("Uploading CV to MinIO:", {
    ...
    try {
      ...
      console.log("✅ CV uploaded successfully to MinIO:", result);
      ...
      try {
        ...
        console.log("✅ Profile CV URL updated successfully");
      } catch (profileError) {
        console.error("❌ Failed to update profile CV URL:", profileError);
      ...
      return NextResponse.json({
        success: true,
        file: {
          id: `${session.user.id}-cv-${timestamp}`,
          name: fileName,
          type: file.type,
          size: file.size,
          url: proxyUrl,
